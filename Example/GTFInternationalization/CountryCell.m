...
- (void)awakeFromNib {
  ...
  self.flagView.gtf_semanticContentAttribute = UISemanticContentAttributeForceLeftToRight;

  // The speech indicator icon can be mirrored, so we keep the default of
  // UISemanticContentAttributeUnspecified.
  //  self.speechIcon.gtf_semanticContentAttribute = UISemanticContentAttributeUnspecified;
  ...
  // gtf_semanticContentAttribute value as the default UISemanticContentAttributeUnspecified.
  //  self.nameLabel.gtf_semanticContentAttribute = UISemanticContentAttributeUnspecified;
  ...
  dispatch_once(&onceToken, ^{
    ...
    if (self.speechIcon.gtf_effectiveUserInterfaceLayoutDirection !=
        UIUserInterfaceLayoutDirectionLeftToRight) {
      sSpeakingIcon = [sSpeakingIcon gtf_imageWithHorizontallyFlippedOrientation];
      sNonspeakingIcon = [sNonspeakingIcon gtf_imageWithHorizontallyFlippedOrientation];
    }
  });
}
...
- (void)layoutSubviews {
  ...
  if (self.gtf_effectiveUserInterfaceLayoutDirection != UIUserInterfaceLayoutDirectionLeftToRight) {
    flagFrame = GTFRectFlippedHorizontally(flagFrame, CGRectGetWidth(self.bounds));
    nameFrame = GTFRectFlippedHorizontally(nameFrame, CGRectGetWidth(self.bounds));
    speechIconFrame = GTFRectFlippedHorizontally(speechIconFrame, CGRectGetWidth(self.bounds));
  }
